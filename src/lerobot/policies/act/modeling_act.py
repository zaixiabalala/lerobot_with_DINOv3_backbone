    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.image_features:
            if config.vision_backbone.startswith("dinov3"):
                # Use DINOv3 encoder
                from lerobot.policies.act.dino.dino_encoder import DINOEncoder
        ...
        if self.config.image_features:
            # For DINOv3, the projection is already done in the encoder, so use identity
            # For ResNet, project from backbone output to dim_model
            if config.vision_backbone.startswith("dinov3"):
